...
public class UserTokenReissueService {
    ...
    public TokenResponseDto reissueTokens(Long userId, String refreshToken) {
        log.info("{ UserTokenReissueService } : 토큰 재발급 진입");
        // 사용자가 보유하고 있는 RefreshToken인지
        if (!tokenService.isRefreshTokenExists(userId, refreshToken)) {
            log.info("{ UserTokenReissueService } : 유저가 보유하고 있는 토큰이 아님");
            throw BaseException.type(AuthErrorCode.AUTH_INVALID_TOKEN);
        }

        String newAccessToken = jwtProvider.createAccessToken(userId, Role.USER);
        String newRefreshToken = jwtProvider.createRefreshToken(userId, Role.USER);
