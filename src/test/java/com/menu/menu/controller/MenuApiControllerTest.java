...
//                .header(AUTHORIZATION, BEARER_PREFIX + ACCESS_TOKEN)
    ...
    class updateMenu {
        ...
        void throwExceptionWhenUserIsNotOwner() throws Exception {
            // given
            verify(menuService, times(1)).updateMenu(anyLong(), anyLong(), anyLong(), any(), any(), anyLong());

            doThrow(BaseException.type(StoreErrorCode.USER_IS_NOT_OWNER))
                    .when(menuService)
                    .updateMenu(anyLong(), anyLong(), anyLong(), any(), any(), anyLong());
    ...
    class deleteMenu {
        ...
        void throwExceptionWhenUserIsNotOwner() throws Exception {
            ...
            doThrow(BaseException.type(StoreErrorCode.USER_IS_NOT_OWNER))
                    .when(menuService)
                    .deleteMenu(anyLong(), anyLong(),anyLong());
            // when
            MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders
